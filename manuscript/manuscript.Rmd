...
## Does FCS have better sensitivity than ORA with real RNA-seq data?

To understand whether FCS has better sensitivity than ORA in a real setting, we conducted a
systematic downsampling of a gene expression dataset cosisting of normal and tumor samples
...
The resource is currently available at https://oratool.ziemann-lab.net/ and available as a
docker container (https://hub.docker.com/repository/docker/mziemann/background_app/general).

ClusterProfiler is not the only tool to suffer from these problems, they are widespread
(**Table 1**). Still, there are some tools free of such problems.
For example, fora is a good option for R-based workflows, however the function does not calculate the fold enrichment score,
...
One problem that all of these tools listed in Table 1 have is that a background list is optional.
Sometimes this feature is hidden under "advanced options".
Given the strong biological and technical biases that are present in contemporary omics such as including single cell and spatial transcriptomics,
...
: Table 1. Classification of several popular freely available ORA tools regarding the provision of information such as
...
The purpose of this work was to thoroughly characterise two subtle problems with ORA, but there are some
bigger problems noted.
Firstly, all variations of ORA were less accurate as compared to FCS in this simulation work, confirming a
...
noisy.
The higher recall of FCS demonstrated in simulated differential expression profiles and confirmed with
downsampled real cancer RNA-seq data means that researchers will have more power to identify pathways
...
on significance scores when interpreting enrichment results.
ORA results can vary dramatically depending on the number of genes selected in the
...
that if the data can be scored/ranked, methods like FCS are recommended over ORA.
...
using the accession numbers mentioned in the Methods section.
...
The Docker image is available on DockerHub (https://hub.docker.com/r/mziemann/background).
The code repository and Docker image have been uploaded to Zenodo for long-term archiving
(https://zenodo.org/record/TODOXXX).
