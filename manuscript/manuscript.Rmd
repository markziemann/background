...
Over-representation analysis (ORA) is used widely to asses for the enrichment of functional
...
These problem can be mitigated by changing packages/websites for ORA or by changing to another
...
p-values were used for ORA, similar to a previous report [@Tarca2013-xh].
...
genes were analysed in parallel by clusterProfiler's `enricher` function, which suffers from
the two aforementioned problems and `fora` which does not.
In parallel, `fgsea` was used to compare the performance of these two ORA methods against a
prototypical FCS method.
ClusterProfiler scored very high precision when added variance was less than 0.2, but showed a
progressive worsening in precision as more noise was added to the dataset, dropping to 0.69
with added variance of 0.5.
In comparison, the precision of fora was relatively stable around 0.97 until added variance
reached 0.5, where it dropped to 0.88.
The recall of ORA methods dropped suddenly when added variance was 0.2 or greater.
fgsea showed stable precision of 0.93 or greater when added variance was 0.5 or lower, which
dropped slightly to 0.91 with added variance of 0.6.
fgsea recall was better than either ORA tool across all levels of added variance.
This result shows that these two subtle problems result in poorer precision for
clusterProfiler, which becomes more severe with higher levels of noise.
Overall, fgsea had similar mean precision to fora (0.94), but the higher recall of fgsea
meant it had a higher overall accuracy measured by F1 score (0.83) as compared to fora and
clusterProfiler (0.61 and 0.64 respectively).
...
The results shown here show that the background problem effects smaller gene set libraries
like KEGG and HPC more severely as compared to Cellmarkers and GO libraries which describe
...
In extreme cases, correction of this problem can lead to around half of the significant
results changing.
...
Although the purpose of this work was to describe two subtle problems with ORA, this work
highlights some major drawbacks.
Firstly, ORA was less accurate as compared to FCS in this simulation work, confirming a
previous report [@Kaspi2020-rn].
Secondly ORA results can vary dramatically depending on the number of genes selected in the
