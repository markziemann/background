...
To generate differential expression profiles, 5% of gene sets were selected to be differentially
expressed, with equal numbers of up- and down-regulated pathways.
Pseudosamples were generated by first downsampling the counts using the `thincounts` function of
the `edgeR` package v4.2.0 [@Robinson2010-eb] to 20M reads, followed by the addition of some extra variation drawn from a
normal distribution with mean of 1 and standard deviation varying between 0.3 and 0.45, this
value selected randomly for each gene.
Five "control" and five "case" pseudosamples were generated.
...
With these expression profiles, DESeq2 was used for differential analysis, and the results were
subjected to `enricher` which suffers from both problems, `fora`, an ORA function of the `fgsea`
bioconductor package v1.30.0 which does not suffer these problems, and `fgsea` a fast
implementation of the preranked GSEA algorithm [@Korotkevich2016-gd].
...
If fewer than 200 genes are detected as significant, then 200 genes with the smallest p-values
...
After enrichment analysis with these approaches, the significant sets up and down regulated sets
were selected and compared to the ground truth in order to calculate precision, recall and F1
score.

At each value of added variance, 500 replications were conducted, each with as set seed for
reproducibility.
