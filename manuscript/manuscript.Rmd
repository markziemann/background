...
Correction of p-values for multiple testing is also crucial in controlling the false positive rate
as enrichment analysis typically involves throusands of parallel tests.
...
If fewer than 200 genes met this criteria, then the 200 genes with the smallest p-values
...
v4.10.0, using a minimum set size of 5 and a no maximum set size.
...
enricher, followed by FDR correction in R, to obtain the properly corrected FDR values.
...
log fold change in the case samples of 0.5 for upregulated genes and -0.5 for downregulated genes.
...
were subjected to `enricher` which suffers from both problems, `fora`, an ORA function of
the `fgsea` bioconductor package v1.30.0 which does not suffer these problems, and `fgsea` a
fast implementation of the preranked GSEA algorithm [@Korotkevich2016-gd].
...
If fewer than 200 genes were detected as significant, then 200 genes with the smallest p-values
were used for ORA, similar to a previous report [@Tarca2013-xh].
...
sets were selected and compared to the ground truth in order to calculate precision, recall
and F1 score.
...
clusterProfiler's `enricher` function with and without correcting the background problem.
...
datasets. Foreground list consisted of 2000 genes with the smallest p-values](../analysis/fig2_ORAfdr.png
"Figure 2A shows that correcting the FDR bug slightly reduces the number of significant sets.")
...
Simulated differential expression profiles with apriori changes to predetermined pathway genes were
analysed in parallel by clusterProfiler's `enricher` function, which suffers from the two
aforementioned problems and `fora` which does not.
In parallel, `fgsea` was used to compare the performance of these two ORA methods against a
prototypical FCS method.
ClusterProfiler scored very high precision when added variance was less than 0.4, but showed a
progressive worsening in precision as more noise was added to the dataset, dropping to 0.61 with added
variance of 0.6.
In comparison, the precision of fora was relatively stable around 0.97 until added variance reached 0.5.
The recall of ORA methods dropped suddenly when added variance was 0.2 or larger.
fgsea showed stable precision of 0.94 when added variance was 0.4 or lower, and dropped slightly to
0.91 with added variance of 0.6.
fgsea recall was better than either ORA tool across all levels of added variance.
This result shows that these two subtle problems result in poorer precision for clusterProfiler,
which becomes more severe with higher levels of noise.
Overall, fgsea had similar precision to fora (mean=0.94), but the higher recall of fgsea meant it had
a higher overall accuracy measured by F1 score (0.83) as compared to fora and clusterProfiler
(0.61 and 0.64 respectively).

![**Figure 3. Comparison of clusterProfiler, fora and fgsea accuracy with simulated data.**
Precision, recall and F1 score are shown with the addition of different amounts of random
noise to expression levels.](../analysis/sim/fig3_sim.png "Figure 3 shows that fora is more
precise than clusterProfiler, and fgsea has higher recall than either ORA methods.")

## Discussion

Here we have described the effect of two subtle problems with common implementations of ORA.
This work was inspired by a doctoral student new to bioinformatics who was puzzled by confusing results
obtained from clusterProfiler when using a custom set of pathways.
Our hunch that clusterProfiler was behaving in an undesired way was confirmed by the discovery of
several posts to online bioinformatics forums dating to ~2020 where confused users were wondering why
the number of background genes changed so much.
ClusterProfiler maintainers recently provided an option to prevent the loss of unannotated genes from
the background `options(enrichment_force_universe = TRUE)`, however this feature isn't yet described
in the official documentation.

The second problem is potentially more serious in that it could lead large numbers false positives
under specific conditions; when the foreground list is short and the median pathway size is relatively
small.
These false positives arise due to an underestimate of the actual number of tests conducted.
ClusterProfiler is not the only tool to suffer from these problems, they are widespread (**Table 1**).
...
Although the purpose of this work was to describe two subtle problems with ORA, it has some major
drawbacks which limit its utility.

* The method is rarely reported comprehensively, especially crucial details about background gene list
and FDR correction.

* Results vary dramatically depending on the number of genes selected in the foreground.

* Overall ORA is less accurate as compared to FCS

These bigger problems suggest FCS should replace ORA in most cases.
Where it makes sense to use ORA would be when FCS is not applicable, for example when investigating
the over-represented categories in gene expression modules defined by packages like WGCNA (Weighted
Gene Correlation Network Analysis)[@Langfelder2008-hu].
