...
standard deviation varying between 0.1 and 1.0, this value selected randomly for each gene.
...
fold change in the case samples of 2.0 for upregulated genes and 0.5 for downregulated genes.
...
and F1 score.
At each value of added variance, 500 replications were conducted.
...
results of ORA of seven datasets with the original and corrected algorithm for nine different
gene set libraries.
...
analysed in parallel by clusterProfilers enricher function, which suffers from the two aforementioned
problems and fora which does not.
In parallel, fgsea was used to compare the performance of these two ORA methods against a prototypical
FCS method.
ClusterProfiler scored very high precision when added variance was less than 0.4, but showed a
progressive worsening in precision as more noise was added to the dataset, dropping to 0.68 with added
variance of 1.0.
In comparison, the precision of fora was relatively stable around 0.97 until added variance reached 0.9.
The recall of ORA methods dropped suddenly when added variance was 0.5 or larger.
fgsea showed stable precision of 0.94 across all levels of added variance, and superior recall overall.
...
Our hunch that clusterProfiler was behaving in an undesired way was confirmed by several posts to
online bioinformatics forums dating to 2020 where confused users were wondering why the number of
background genes changed so much.
ClusterProfiler maintainers recently provided an option to prevent the loss of unannotated genes from the
background `options(enrichment_force_universe = TRUE)`, however this feature isn't yet described in the
official documentation.

The second problem is potentially more serious in that it could lead to lots of false positives,
as clusterProfiler underestimates the actual number of tests that were conducted.
As shown in Figure 2B, this affects results differently depending on the gene set library used and also
the number of genes selected in the foreground list.
...
The reason why such problems proliferated for so long is that using a custom background for ORA seems
to be the exception, rather than the rule.
Several tools do not even allow for the use of a custom background, suggesting that some tool developers
were unaware of the seriousness of this error.
This is one reason why ORA tool documentation also seems to lack any explanation of the importance of
using a custom background set.
