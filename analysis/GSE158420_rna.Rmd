...
fres <- lapply(sizes, function(size) {
...
rownames(fres) <- sizes
colnames(fres) <- c("up","dn")
fres <- as.data.frame(fres)
fres$tot <- fres$up + fres$dn
tot <- fres$tot
names(tot) <- sizes
...
dots <- lapply(ssres,function(x) { x[,"TP"] })
lapply(dots,summary)
boxplot(dots,names=SAMPLESIZES,col="white",ylab="no. consistent",xlab="sample size",cex=0,main="fora",ylim=c(0,nrow(osig))) ; grid()
beeswarm(dots,add=TRUE,pch=1,cex=0.6)
abline(h=nrow(osig))
