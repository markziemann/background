...
pdf("fig1_ORAsig.pdf",width=7,height=7)
par(mar=c(c(5.1, 7.1, 2.1, 2.1) ))
barplot(t(df),beside=TRUE,horiz=TRUE,las=1,xlim=c(0,1250),
  xlab="no. significant sets",
  col=c("gray30","gray80"))
...
pdf("fig1_ORAjac.pdf",width=7, height=7)
par(mar=c(c(5.1, 5.1, 2.1, 2.1) ))
plot(cov,jac2,xlim=c(0,30000),ylim=c(0,1),pch=19,col="lightgray",cex=3,
  xlab="gene coverage",ylab="Jaccard index")
text(cov,jac2,labels=names(cov))
...
# with SD
mysd <- apply(jac,2,function(x) { sd(x,na.rm=TRUE) } )
par(mar=c(c(5.1, 5.1, 2.1, 2.1) ))
plot(cov,jac2,xlim=c(0,30000),ylim=c(0,1),pch=19,col="lightgray",cex=3,
  xlab="gene coverage",ylab="Jaccard index")
text(cov,jac2,labels=names(cov))
...
HEADER=paste("Rsq =",RSQ,"; p-val =",PVAL)
mtext(HEADER)
arrows(x0=cov,x1=cov,y0=jac2+mysd,y1=jac2-mysd,code=3, angle=90, length=0.1 , col="gray40")

png("fig1_ORAjacsd.png")
par(mar=c(c(5.1, 5.1, 2.1, 2.1) ))
plot(cov,jac2,xlim=c(0,30000),ylim=c(0,1),pch=19,col="lightgray",cex=3,
  xlab="gene coverage",ylab="Jaccard index")
text(cov,jac2,labels=names(cov))
...
pdf("fig1_ORAjacsd.pdf",width=7, height=7)
par(mar=c(c(5.1, 5.1, 2.1, 2.1) ))
plot(cov,jac2,xlim=c(0,30000),ylim=c(0,1),pch=19,col="lightgray",cex=3,
  xlab="gene coverage",ylab="Jaccard index")
text(cov,jac2,labels=names(cov))
...
## Characterise sets specific to original and uncorrected analysis
...
compare_ora2 <- function(bad,good){
  ...
  result=list("intersect"=itx,"spec_good"=spec_good)
  return(result)
}
...
ora_dat3 <- mclapply(1:nrow(params), function(i) {
  j=params[i,1]
  k=params[i,2]
  bad <- orabad(d[[j]],gs[[k]])
  good <- oragood(d[[j]],gs[[k]])
...
speclen <- lapply(ora_dat3, function(j) {
  itxlen <- mean(gslengths[which(names(gslengths) %in% j[[1]])])
  speclen <- mean(gslengths[which(names(gslengths) %in% j[[2]])])
  return(c("itxlen"=itxlen,"spec"=speclen))
})

itxlenmx <- matrix(sapply(speclen,"[[",1),nrow=7)
speclenmx <- matrix(sapply(speclen,"[[",2),nrow=7)

rowMeans(itxlenmx,na.rm=TRUE)

rowMeans(speclenmx,na.rm=TRUE)

( rowMeans(speclenmx,na.rm=TRUE) - rowMeans(itxlenmx,na.rm=TRUE) ) / rowMeans(itxlenmx,na.rm=TRUE)

mean( ( rowMeans(speclenmx,na.rm=TRUE) - rowMeans(itxlenmx,na.rm=TRUE) ) / rowMeans(itxlenmx,na.rm=TRUE) )

```

Overall, the size of gene sets specific to the "fixed" analysis were 9.3% smaller than
the intersect.
...
gslibs <- unique(params2$Var2)

nsl <- lapply(gslibs,function(g) {
  orig <- subset(res1,Var2==g)$nsig_bad
  corr <- subset(res1,Var2==g)$nsig_good
  df1 <- data.frame("gsl"=g,"type"="orig","count"=orig)
  df2 <- data.frame("gsl"=g,"type"="corr","count"=corr)
  return(rbind(df1,df2))
} )

nsl <- do.call(rbind,nsl)
nsl$set <- paste(nsl$gsl,nsl$type)

unsl <- unstack(nsl[,c(3,4)])

#rearrange KEGG, Hallmark, HPO, Wikipath, miR, TFT, Reactome, Cellmarkers, GO
unsl <- unsl[,c(10,9, 6,5, 8,7, 18,17, 12,11, 16,15, 14,13, 2,1, 4,3)]

par(mar = c(5.1, 8.1, 4.1, 2.1))
library(beeswarm)
boxplot(unsl,horizontal=TRUE,las=1,col=c("pink","lightblue"),cex=0,xlab="no. significant sets FDR<0.05")
beeswarm(unsl,horizontal=TRUE,add=TRUE,col=c("red","blue"),pch=19,cex=0.6)
beeswarm(as.list(colMeans(unsl)),horizontal=TRUE,add=TRUE,col="black",pch="x",cex=1.0)

unsl1 <- unsl+1
boxplot(unsl1,horizontal=TRUE,log="x",las=1,col=c("pink","lightblue"),cex=0,xlab="no. significant sets FDR<0.05*")
beeswarm(unsl1,horizontal=TRUE,add=TRUE,col=c("red","blue"),pch=19,cex=0.6)
beeswarm(as.list(colMeans(unsl1)),horizontal=TRUE,add=TRUE,col="black",pch="x",cex=1.0)

legend("bottomright", inset=.02, legend=c("corrected","original"),
  fill=c("lightblue","pink"),col=c("blue","red"), horiz=FALSE, cex=1.1)

png("fig1_ORAsigbox.png")
boxplot(unsl1,horizontal=TRUE,log="x",las=1,col=c("pink","lightblue"),cex=0,xlab="no. significant sets FDR<0.05*")
beeswarm(unsl1,horizontal=TRUE,add=TRUE,col=c("red","blue"),pch=19,cex=0.6)
beeswarm(as.list(colMeans(unsl1)),horizontal=TRUE,add=TRUE,col="black",pch="x",cex=1.0)
legend("bottomright", inset=.02, legend=c("corrected","original"),
...
pdf("fig1_ORAsigbox.pdf",width=7, height=7)
par(mar = c(5.1, 8.1, 4.1, 2.1))
boxplot(unsl1,horizontal=TRUE,log="x",las=1,col=c("pink","lightblue"),cex=0,xlab="no. significant sets FDR<0.05*")
beeswarm(unsl1,horizontal=TRUE,add=TRUE,col=c("red","blue"),pch=19,cex=0.6)
beeswarm(as.list(colMeans(unsl1)),horizontal=TRUE,add=TRUE,col="black",pch="x",cex=1.0)
...
## Focus on dataset 1 down with HPO genesets

This extreme example will help us understand the bias better.

```{r,hpodn}

orig <- orabad(d[[1]],gs[[7]])
corr <- oragood(d[[1]],gs[[7]])
orig_dn <- orig[[2]]
corr_dn <- corr[[2]]
mg <- merge(orig_dn,corr_dn,by=0)

mg$size <- as.numeric(sapply(strsplit(mg$BgRatio.x,"/"),"[[",1))
quantiles <- quantile(mg$size, prob = seq(0, 1, length = 6), type = 5)
quantiles
mg$levels <- cut2(mg$size, cuts = as.numeric(quantiles))
cols <- gsub("5","red",gsub("4","orange",gsub("3","yellow",gsub("2","green",gsub("1","blue",as.character(as.numeric(mg$levels)))))))

plot(-log10(mg$pvalue.x),-log10(mg$pvalue.y),
  ylim=c(0,7), xlim=c(0,7), pch=19,
  main="SRP128998 HPO down",
  xlab="-log10 p-value (original)", ylab="-log10 p-value (corrected)")

grid()
abline(coef = c(0,1))

points(-log10(mg$pvalue.x),-log10(mg$pvalue.y),
...
# PDF
pdf("fig1_HPOdn.pdf",width=7, height=7)
plot(-log10(mg$pvalue.x),-log10(mg$pvalue.y),
  ylim=c(0,7), xlim=c(0,7), pch=19,
  main="SRP128998 HPO down",
  xlab="-log10 p-value (original)", ylab="-log10 p-value (corrected)")
...
library(reshape2)
res2a <- as.matrix(acast(res1, Var1~Var2, value.var="nsig_bad"))
rownames(res2a) <- gsub("bulkrna","d",rownames(res2a))
rownames(res2a) <- gsub(".Rds"," original",rownames(res2a))

res2b <- as.matrix(acast(res1, Var1~Var2, value.var="nsig_good"))
rownames(res2b) <- gsub("bulkrna","d",rownames(res2b))
rownames(res2b) <- gsub(".Rds"," corrected",rownames(res2b))

res2 <- rbind(res2a,res2b)
res2 <- res2[order(rownames(res2)),]
res2c <- rowMeans(res2)
res2 <- cbind(res2,res2c)

res2 %>%
  kbl(row.names = TRUE, caption="Effect of background problem correction across seven studies and nine gene set libraries") %>%
  kable_paper("hover", full_width = F)

saveRDS(res2,"bgprob.rds")

rowMeans( ( res2b - res2a ) / res2a ) * 100
...
save.image("analyse_rnaseq1.Rdata")
