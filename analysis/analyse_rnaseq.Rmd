...
orabad <- function( df, gs ) {
  ...
  ora_up <- as.data.frame(enricher(gene = defup ,
    universe = bg,  maxGSSize = 500000, TERM2GENE = gs1,
...
signif(jac,3) %>%
  kbl(row.names = FALSE, caption="Jaccard similarity Corrected:Original") %>%
  kable_paper("hover", full_width = F)
...
barplot(sort(jac2),horiz=TRUE,las=1,xlim=c(0,1),
  main="Jaccard similarity Corrected:Original",xlab="fold increase")
